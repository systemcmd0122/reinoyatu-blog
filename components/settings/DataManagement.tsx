  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {[
          { label: "記事", value: stats.blogsCount, icon: FileText, color: "from-blue-500 to-cyan-500" },
          { label: "シリーズ", value: stats.collectionsCount, icon: Layers, color: "from-purple-500 to-pink-500" },
          { label: "画像", value: stats.imagesCount, icon: ImageIcon, color: "from-green-500 to-emerald-500" },
          { label: "合計反応", value: stats.likesCount + stats.bookmarksCount, icon: AlertTriangle, color: "from-orange-500 to-yellow-500" },
        ].map((item, i) => (
          <div key={i} className="relative group">
            <div className={`absolute inset-0 bg-gradient-to-br ${item.color} opacity-0 group-hover:opacity-10 transition-opacity rounded-[2rem] blur-xl`} />
            <Card className="border-border/50 shadow-sm overflow-hidden rounded-[2rem] hover:border-primary/20 transition-all duration-300">
      ...
      <Tabs defaultValue="blogs" className="w-full">
        <TabsList className="grid grid-cols-3 w-full max-w-md mb-12 bg-muted/50 p-1.5 rounded-2xl border border-border/50">
        ...
        <TabsContent value="blogs" className="space-y-4">
          <div className="bg-card border border-border/50 rounded-[2rem] overflow-hidden shadow-sm divide-y divide-border/50">
            {blogs.length > 0 ? (
              <>
                {blogs.map(blog => (
                  <div key={blog.id} className="p-6 flex items-center justify-between hover:bg-muted/30 transition-all duration-300 group">
                    <div className="min-w-0 flex-1 pr-4">
                      <h4 className="font-black text-lg truncate group-hover:text-primary transition-colors">{blog.title}</h4>
        ...
        <TabsContent value="collections" className="space-y-4">
          <div className="bg-card border border-border/50 rounded-[2rem] overflow-hidden shadow-sm divide-y divide-border/50">
            {collections.length > 0 ? (
              <>
                {collections.map(col => (
                  <div key={col.id} className="p-6 flex items-center justify-between hover:bg-muted/30 transition-all duration-300 group">
                    <div className="min-w-0 flex-1 pr-4">
                      <h4 className="font-black text-lg truncate group-hover:text-primary transition-colors">{col.title}</h4>
                      <div className="flex items-center gap-3 mt-1.5">
                        <span className="text-[10px] font-bold text-muted-foreground flex items-center gap-1">
                          <Clock className="h-3 w-3" />
                          {formatJST(col.created_at)}
                        </span>
                        {col.is_public ? (
                          <span className="text-[8px] font-black uppercase tracking-tighter bg-primary/10 text-primary px-1.5 py-0.5 rounded">Public</span>
                        ) : (
