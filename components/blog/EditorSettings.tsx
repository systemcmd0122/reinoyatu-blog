...
const EditorSettings: React.FC<EditorSettingsProps> = ({
  ...
  const applyAiSuggestion = () => {
    ...
    if (aiSuggestion.type === "summary") {
      form.setValue("summary", aiSuggestion.content as string, { shouldValidate: true, shouldDirty: true });
    ...
    <div className="space-y-10">
      ...
      <section className="space-y-4">
        ...
        <div className="group relative aspect-video rounded-xl overflow-hidden border-2 border-dashed border-muted hover:border-primary/50 transition-all cursor-pointer bg-muted/20">
          {imagePreview ? (
            <>
              ...
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                <Button size="sm" variant="secondary" onClick={() => document.getElementById('settings-image-upload')?.click()}>
                  変更
                </Button>
            ...
            <label htmlFor="settings-image-upload" className="absolute inset-0 flex flex-col items-center justify-center text-center p-4 cursor-pointer">
          ...
          <input id="settings-image-upload" type="file" className="hidden" accept="image/*" onChange={handleImageUpload} />
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        <Textarea
          ...
          onChange={(e) => form.setValue("summary", e.target.value, { shouldDirty: true })}
